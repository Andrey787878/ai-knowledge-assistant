...
- автоматический полный деплой для `push` в `main`, которые затрагивают
  релизные Kubernetes/CD-файлы и уже прошли `CI`;
...
- `cd-k3s-auto.yml` — полный deploy после успешного `CI` на актуальном
  коммите в `main`, но только для релизных Kubernetes/CD-изменений;
