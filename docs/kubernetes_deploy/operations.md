# Эксплуатация и приемка этапов B и C

## Что описывает документ

Документ описывает рабочий цикл Kubernetes-контура этапов B и C: в каком
порядке запускать инфраструктурный и Kubernetes-деплой, какие проверки
выполнять перед релизом и что считать успешной приемкой.
...
Релиз Kubernetes-контура считается успешным только если одновременно выполняется:
