...
6. после успешного `CI` выполняются:
   - `deploy-k3s-scope.sh all`
   - `smoke-k3s-scope.sh all`
7. после успешной выкладки SHA сохраняется как последний полный успешный deploy;
8. итог записывается в `GITHUB_STEP_SUMMARY`.
