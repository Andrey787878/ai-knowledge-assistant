# SLI и SLO этапов B и C

## Что описывает документ

Этот документ фиксирует набор SLI и SLO для Kubernetes-контура этапов B и C.
Он опирается только на те сигналы, которые уже есть в кластере и доведены в
репозитории до рабочего состояния. Здесь нет метрик, которые пришлось бы
добавлять в документацию без реального источника данных.
...
`N8nIngressLatencyP95High`, если `n8n` деградирует из-за отказа `redis`.
`PostgresDown` делает то же для `n8n`, а также подавляет `WikiDown`,
`WikiPublicEndpointDown`, `WikiIngress5xxHigh` и
`WikiIngressLatencyP95High`, если `wiki` ломается вслед за недоступным
`postgres`. Это разделяет причину и следствие: в email остается корневой
инцидент зависимости, а производные симптомы остаются видны в UI для
расследования, но не превращаются в отдельные письма.
