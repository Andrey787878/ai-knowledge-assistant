...
Документ описывает выпуск, продление и диагностику TLS-сертификатов на
публичном edge-хосте этапа A. Здесь собраны практические команды для проверки
`certbot`, файлов сертификата и доступности доменов `wiki` и `n8n`.
