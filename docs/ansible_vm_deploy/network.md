...
В этапе A сеть построена по принципу минимальной публичной поверхности и
многоуровневой защиты:

- снаружи доступен только публичный edge/bastion-хост `wiki`,
- приватные VM `db`, `n8n`, `ollama` не имеют public IP,
- входящие потоки ограничены на нескольких слоях: Security Groups, host
...
### Уровень edge-прокси (`nginx` allow-list)
...
Итог: доступ к UI ограничен не только Security Groups и firewall, но и на
...
- SSH на `wiki` разрешен только из `firewall_admin_ssh_sources`.
...
- `80/tcp` используется для ACME HTTP-01 и перенаправления `HTTP -> HTTPS`.
...
- egress в Security Groups оставлен `ANY`,
- приватные VM изолированы отсутствием public IP и выходят в интернет через NAT.

Это осознанный компромисс этапа A: жесткий входящий периметр плюс
