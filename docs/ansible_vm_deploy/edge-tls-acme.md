...
Документ описывает, как в этапе A устроен публичный TLS для edge-хоста
`wiki` (`wiki_hosts`). Здесь зафиксированы домены, порядок bootstrap и
механика ACME HTTP-01.
...
- сертификаты, которым доверяет браузер, для доменов `wiki` и `n8n`;
- автоматическое продление без ручного перевыпуска;
- привязку edge proxy к актуальным путям Let's Encrypt.
