...
- сначала базовая подготовка и Docker Engine на всех хостах,
- потом stateful-слой БД и резервное копирование,
...
- Что меняет: пакеты, пользователей, SSH-конфигурацию, политику firewall, compose-стеки, `nginx` и `certbot`.
...
- Что делает:
  - проверяет внешнюю HTTPS-доступность `wiki` и `n8n` с управляющей машины,
  - проверяет HTTP-01 challenge path,
  - проверяет edge redirect и HTTPS health,
  - проверяет связность `n8n -> ollama` и `n8n -> postgres`,
  - проверяет deny-by-default для `db -> n8n:5678` и `db -> ollama:11434`,
  - делает e2e вызов `agent-query`,
  - проверяет, что память сессии записалась в PostgreSQL.
...
- Что меняет:
  ...
  - `certbot.timer` и sync переменных cert/key для edge роли.
- Что проверяет:
